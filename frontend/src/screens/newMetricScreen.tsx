...
const NewMetricScreen: React.FC<Props> = ({ navigation }) => {
    ...
    const [newMetricType, setNewMetricType] = useState<MetricTypes>(MetricTypes.Label)
    ...
    const metricType = (metricType: string) => {
        return (
            ...
            >
                <Ionicons name={MetricTypeIcons[metricType as keyof typeof MetricTypes]} size={24} />
                <Text>{metricType}</Text>
            </TouchableOpacity>
        )
    }

    const metricItem = ({ item, drag, isActive }: RenderItemParams<RoboMetric>) => {
        const Metric = () => {
            switch (item.type) {
                case MetricTypes.Label:
                    return (
                        <LabelMetric metric={item} />
                    )
                case MetricTypes.Text:
                    return (
                        <TextMetric metric={item} />
                    )
                case MetricTypes.Counter:
                    return (
                        <CounterMetric metric={item} />
                    )
    ...
    const dialogCreatePressed = () => {
        setNewMetricName("")
        setMetricItems(old => [...old, new RoboMetric(newMetricName, newMetricType)])
        setShowDialog(false)
    }
